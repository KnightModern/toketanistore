package com.toketanistore.app.Adapter;
...
public class My_Nextday_Order_Adapter extends RecyclerView.Adapter<My_Nextday_Order_Adapter.MyViewHolder> implements Filterable {

    String select = "false";
    SharedPreferences preferences;
    private List<NextdayOrderModel> modelList;
    private List<NextdayOrderModel> searchList;
...
//            holder.Assign_Boy_name.setVisibility(View.VISIBLE);
        ...
        holder.paymentStatus.setText(mList.getPayment_status());
...
//                todayOrderClickListner.ClickGetBoys(position, "unassign");
        ...
        holder.Assign_Order_button.setOnClickListener(v -> todayOrderClickListner.assignClickBoys(holder.getAdapterPosition(), "tomorrow"));
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence charSequence) {
                ...
                if (charString.isEmpty()) {
                    ...
                    List<NextdayOrderModel> filteredList = new ArrayList<>();
                    for (NextdayOrderModel row : searchList) {
                        if (row.getUser_name().toLowerCase().contains(charString.toLowerCase()) || row.getCart_id().contains(charSequence)) {
            ...
            protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
                modelList = (ArrayList<NextdayOrderModel>) filterResults.values;
                notifyDataSetChanged();
            }
        };
    }

    public void setDataAfterSearch(List<NextdayOrderModel> nextDayOrders) {
        modelList = new ArrayList<>();
        searchList = new ArrayList<>();
        modelList.clear();
        searchList.clear();
        modelList.addAll(nextDayOrders);
        searchList.addAll(nextDayOrders);
        notifyDataSetChanged();
    }
